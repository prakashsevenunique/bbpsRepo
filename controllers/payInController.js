...
exports.allPayin = async (req, res) => {
  try {
    const {
      keyword,
      status,
...
exports.createPayIn = async (req, res) => {
  try {
    ...
    // Ensure no duplicate reference
    const existing = await PayIn.findOne({ reference });
    if (existing) {
...
exports.generatePayment = async (req, res) => {
    ...
    try {
        ...
        console.error("Error generating payment intent:", error.message);
        if (error.response) {
            return res.status(error.response.status).json({
                success: false,
                message: error.response.data.message || "External API error occurred",
            });
        }
